...
//Move the dot left or right
	int num = 0;
	/*if(mPosY <= y)
		randVelY = sqrt(2/(pow((mPosX-x)/(mPosY-y), 2)+1));
	else
		randVelY = -1*sqrt(2/(pow((mPosX-x)/(mPosY-y), 2)+1));
	randVelX = ((mPosX-x)/(mPosY-y))*(randVelY);
	if((mPosX <= x && randVelX  < 0) || (mPosX > x && randVelX > 0))
		randVelX = -1*randVelX;
	*/
    mPosX += randVelX;
    shiftColliders();
